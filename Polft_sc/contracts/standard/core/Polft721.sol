...
import "../../interfaces/IPolft721.sol";
    ...
    WhitelistUpgradeable
{
    ...
    modifier onlyNftOwner(uint256 _tokenId) {
        require(
            ...
            "Polft721: You are not owner of NFT"
        );
        _;
    }
    ...
    ) external payable validateGranterOnPerson(getAddressWithSignature(_signature, _req)) override(IPolft721) {
        require (msg.value >= mintFee, "Polft721: Not enough fee");
        (bool success, ) = _treasuryAddress.call{value: msg.value}("");
        require(success, "Polft721: Can not send money");
    ...
    function claim(address _to, string memory _ipfs) external override(IPolft721) {
        require (msg.sender == _airdropAddress, "Polft721: not airdrop address");
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
